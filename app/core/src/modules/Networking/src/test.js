import WSModule from './WSModule.js';
...
setTimeout(() => {
	...
	setTimeout(() => {
		...
		if (clientReceived?.error) {
			...
			// Convertimos los buffers a texto legible
			const readableClientMessages = clientReceived.map(item => item.value.toString('utf-8'));
			console.log('\n📥 Cliente recibió:', readableClientMessages);
		}
		...
		if (serverReceived?.error) {
			...
			// Convertimos los buffers a texto legible
			const readableServerMessages = serverReceived.map(item => item.value.toString('utf-8'));
			console.log('📥 Servidor recibió:', readableServerMessages);
		}
	}, 500);
